...
function inject() {
	return new Promise((resolve, reject) => {
		...
		// ping content script
		executeScript({
			code: "chrome.runtime.sendMessage({ loaded: typeof(contentScriptIsLoaded) !== 'undefined' });",
		});
	});
}
	...
	return new Promise((resolve, reject) => {
		if (tabId) {
			chrome.tabs.sendMessage(tabId, { data: msg }, (data) =>
				chrome.runtime.lastError ? console.warn(chrome.runtime.lastError.error) : resolve(data)
			);
		} else {
